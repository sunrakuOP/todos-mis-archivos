...
class Producto{
    ...
    Producto& operator=(const Producto& otro) {
        ...
        stock = otro.stock;
        identificador = otro.identificador;
        std::cout<<"Producto actualizado"<<std::endl; // como se llama nombre anterior y nuevo?
        return *this;
    }
    void mostrar() {
    ...
    bool esIgual(const Producto& otro) {
        ...
        std::cout<<"No son el mismo producto"<<std::endl;
        return false;
    }
...
int main(){
    ...
    p4 = p2;
    p4.mostrar();
    // usar los otro metodos
    p4.esIgual(p2);
    p1.fusionar(p2);
    p1.masBarato(p2);
    // como muestro todos los productos? uno por uno?
    p1.mostrar();
    p2.mostrar();
    p3.mostrar();
    p4.mostrar();
